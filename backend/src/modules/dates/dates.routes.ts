...
import { CONNECTION_STATUS, PLAN_TYPES } from "../../config/constants";
...
import { mustParticipate } from "../matches/matches.routes";
...
datesRouter.post("/matches/:id/proposals", validate(proposeSchema), async (req, res, next) => {
  try {
    ...
    const scheduledAt = new Date((req.body as z.infer<typeof proposeSchema>).scheduledAt);
...
datesRouter.post(
  "/proposals/:id/cancel",
  validate(z.object({ reason: z.enum(["SOMETHING_CAME_UP", "CHANGED_MIND", "UNCOMFORTABLE", "OTHER"]).optional() })),
  async (req, res, next) => {
    try {
      const userId = userIdOf(req);
      const proposal = await prisma.dateProposal.findUnique({ where: { id: req.params.id } });
...
datesRouter.post(
  ...
  validate(z.object({ sawEachOther: z.boolean() })),
  async (req, res, next) => {
    try {
      const userId = userIdOf(req);
      const proposal = await prisma.dateProposal.findUnique({ where: { id: req.params.id } });
...
datesRouter.post(
  ...
  validate(z.object({ otherAppeared: z.boolean() })),
  async (req, res, next) => {
    try {
      const userId = userIdOf(req);
      const proposal = await prisma.dateProposal.findUnique({ where: { id: req.params.id } });
