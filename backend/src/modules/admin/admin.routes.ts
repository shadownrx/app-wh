...
adminRouter.post(
  "/users/:id/status",
  validate(z.object({ status: z.enum(["ACTIVE", "SUSPENDED", "BANNED"]), reason: z.string().optional() })),
  async (req, res, next) => {
    try {
...
adminRouter.post(
  "/users/:id/verify",
  validate(z.object({ method: z.enum(["EMAIL", "PHONE", "SELFIE", "MANUAL"]).default("MANUAL") })),
  async (req, res, next) => {
    try {
      const adminId = userIdOf(req);
...
adminRouter.post(
  "/users/:id/wallet",
  validate(z.object({ amount: z.number().int(), reason: z.string().min(3) })),
  async (req, res, next) => {
    try {
      const adminId = userIdOf(req);
      await adminAdjust({
