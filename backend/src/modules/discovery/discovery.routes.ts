...
import { isBlocked } from "../users/users.routes";
...
discoverRouter.post(
  "/swipe",
  validate(z.object({ targetId: z.string().uuid(), action: z.enum(["LIKE", "PASS", "SUPER_INVITE"]) })),
  async (req, res, next) => {
    try {
      const userId = userIdOf(req);
...
async function blockedSet(userId: string): Promise<string[]> {
